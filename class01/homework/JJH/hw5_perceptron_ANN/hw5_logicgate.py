...
AND_gate = logicGate("AND", xdata, tdata_AND)
OR_gate = logicGate("OR", xdata, tdata_OR)
NAND_gate = logicGate("NAND", xdata, tdata_NAND)
...
def XOR(xdata):
    result = []
    for x in xdata:
        _, nand_result = NAND_gate.predict(x)
        _, or_result = OR_gate.predict(x)
        _, and_result = AND_gate.predict(np.array([nand_result, or_result]))
        result.append(and_result)
    return result

# Test XOR
xdata = np.array([[0,0],[0,1],[1,0],[1,1]])
xor_results = XOR(xdata)
for x, res in zip(xdata, xor_results):
    print(f"{x} : {res}")